...
class PickController(BaseController):
    ...
    def __init__(
        ...
        self._current_random_offsets = self._generate_random_offsets()
                # --- NEW: capture (1 Hz in events 1 & 2) ---
        self._capture_cb = None          # 用户传进来的截图函数：callable(event:int, index:int) -> None
        self._capture_accum = 0.0        # 当前相位内的累计时间（仿真秒）
        self._capture_index = 0          # 已保存的图片序号（递增）
    ...
    def _execute_phase(self, picking_position, end_effector_orientation, current_joint_positions, object_name, gripper_control, gripper_position, gripper_distances):
        ...
        if self._event == 0:
            ...
            picking_position[0] += self._current_random_offsets['x'] *10
            picking_position[1] += self._current_random_offsets['y'] *10 
            picking_position[2] += self._current_random_offsets['z'] *10
            # 标记异常：应用随机偏移导致夹取位置错误
    ...
    def get_aim_error_info(self):
        ...
        if self._exception_fired and not self._exception_reported:
            ...
            info = {
                "type": "pick_aim_error",
                "message": "Pick position error due to random offset in aiming",
                "suffix": "erro",
                "color": (0, 0, 255),
            }
            return info

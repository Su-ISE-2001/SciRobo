...
class PickControllerFailGrasp(BaseController):
    """
    夹取控制器，模拟夹取失败的情况。
    机器人在夹取时会使用错误的位置（添加偏移），导致无法成功夹取物体。
    """
    def __init__(
        self,
        name: str,
        ...
        position_offset: float = 0.05  # 位置偏移量（米），用于模拟夹取失败
    ) -> None:
    ...
    def _execute_phase(self, picking_position, end_effector_orientation, current_joint_positions, object_name, gripper_control, gripper_position, gripper_distances):
        ...
        if self._event == 0:
            ...
            # 阶段2：定位末端执行器进行抓取（添加位置偏移，导致夹取失败）
            # 在Y方向添加偏移，模拟夹取位置错误
            picking_position[1] += self.position_offset / get_stage_units()  # 添加Y方向偏移
            picking_position[0] += self.position_offset * 0.6 / get_stage_units()  # 添加X方向偏移
    ...
    def get_grasp_error_info(self):
        ...
        if self._exception_fired and not self._exception_reported:
            ...
            info = {
                "type": "grasp_error",
                "message": f"Grasp failed: position offset {self.position_offset*1000:.1f}mm",
                "suffix": "erro",
                "color": (0, 0, 255),
            }
            return info

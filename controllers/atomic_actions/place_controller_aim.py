...
class PlaceController(BaseController):
    ...
    def forward(
        ...
        if self._event == 0:
            ...
            self.target_position[0] += self._current_random_offsets['x'] *20
            self.target_position[1] += self._current_random_offsets['y'] *20 
            self.target_position[2] += self._current_random_offsets['z'] *20
            target_joint_positions = self._cspace_controller.forward(
                target_end_effector_position=self.target_position,
                target_end_effector_orientation=end_effector_orientation
            )

...
def parse_args():
    ...
    parser.add_argument('--no-dataset', action='store_true',
                       help='Disable dataset saving (only keep screenshots)')
...
def save_camera_screenshots(camera_images, camera_names, cfg, episode_num, frame_count, suffix="", annotation_text=None, annotation_color=(0, 0, 255)):
    ...
    if saved_paths:
        print(f"已保存截图到 {episode_screenshot_dir}")
...
def main():
    ...
    # 允许动态新增 enable_dataset 配置
    OmegaConf.set_struct(cfg, False)
    # 默认开启数据集保存，如需关闭可通过 --no-dataset 或配置文件 enable_dataset: false
    if not hasattr(cfg, "enable_dataset"):
        cfg.enable_dataset = True
    if args.no_dataset:
        cfg.enable_dataset = False
    ...
    if args.no_video:
        ...
        save_video = True
        # 在headless模式下，不能显示视频窗口，但可以保存视频和截图
    ...
    while simulation_app.is_running():
        ...
        if world.is_playing():
            ...
            annotation_text = None  # 关闭图像红色文字标注，仅保留文件名后缀
            annotation_color = (0, 0, 255)
            exception_suffix = ""
            if frame_exception:
                # 不叠加文本到图像上，仅用于文件名后缀（例如追加 erro）
